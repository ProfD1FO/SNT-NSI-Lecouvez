...
Les données et valeurs associées aux variables en Python peuvent prendre différents types en fonction des besoins. On a par exemple souvent besoin de manipuler des nombres entiers ou des mots. Pour cela Python propose différents types de base possédant chacun leurs particularités et opérations.

### Les entiers (int)

En python le type entier ou (int) correspond aux nombres entiers. 

### Les flottants (float)

Le type flottant permet de représenter les nombres décimaux (à virgule). La virgule est ici remplacée par un point. Par exemple, `2.5` est un flottant. 
...
Ce type permet de représenter les caractères et différents mots ou phrases. Il est évidemment important de pouvoir les manipuler en informatique. Par exemple `"Bonjour"` est une chaîne de caractères.
...
Ce type est assez particulier puisqu'il ne peut prendre que deux valeurs : Vrai ou Faux (`True` et `False` en anglais). Il est très utile, en particulier lorsque l'on utilise des conditions (voir le prochain cours). 
